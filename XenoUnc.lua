-- ENV by Dert
print("Environment loaded. Made by Dert.")

getgenv().IS_DERT_LOADED = false

local oldr = request 
getgenv().request = function(options)
    if options.Headers then
        options.Headers["User-Agent"] = "Dert/RobloxApp/1.0"
    else
        options.Headers = {["User-Agent"] = "Dert/RobloxApp/1.0"}
    end
...
getgenv().saveinstance = newcclosure(function(options)
    ...
    print("saveinstance Powered by UniversalSynSaveInstance")
...
getgenv().savegame = newcclosure(function(options)
    options = options or {}
    assert(type(options) == "table", "invalid argument #1 to 'saveinstance' (table expected, got " .. type(options) .. ") ", 2)
    print("savegame Powered by UniversalSynSaveInstance")
    synsaveinstance(options)
end)

-- Decompiler placeholders (Standard API usage)
...
getgenv().messagebox = newcclosure(function(text, caption, _type)
    ...
    game:GetService("LinkingService"):OpenUrl(game:GetService("ScriptContext"):SaveScriptProfilingData(ps, "dert.messagebox.bat"))
...
getgenv().getscriptfunction = function(script)
    ...
    elseif success then
        ...
        return function()
            return nil, "Dert: Script closure access is restricted."
        end
    end
end
...
local __supported_functions_dert_x64_engine = {
    "request",
    "newcclosure",
...
local __unique_dert_functions_windows_x64_engine = {
}
...
getgenv().getsupportedfunctions = function()
    return __supported_functions_dert_x64_engine
end

getgenv().getuniquefunctions = function()
    return __unique_dert_functions_windows_x64_engine
end
...
getgenv().require = function(scr)
    ...
    if (type(scr) == "number") then 
        if not game:GetObjects('rbxassetid://' .. scr)[1] then 
            warn("Dert: Require failed: invalid asset ID")
            return 
        end
        if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
            if game:GetObjects('rbxassetid://' .. scr)[1].Name == "MainModule" then 
                if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
                    ...
                    warn("Dert: Require failed: cant require a modulescript with no code")
                end
            else 
                warn("Dert: Require failed: require asset id failed")
            end
        end
        return
    end
...
function check(funcName, func, testfunc)
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "Dert", "getaffiliateid function test failed")
end)
...
function Bridge:InternalRequest(body, timeout)
    ...
    if body.Url then
        ...
        error("[Dert Error]: Unknown error", 2)
        return
    end
    ...
    if success and result then
        error("[Dert Error]: " .. tostring(result), 2)
    end

    error("[Dert Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
    ...
    return {
        Success = false,
        StatusMessage = "[Dert Error]: webServer connection failed:  " .. self.serverUrl,
        StatusCode = 599;
...
if not shared.vulnsm then 
    ...
    if type(real) == "userdata" then
        ...
        if table.find(vulnFuncTbl, k) then 
            return function()
            error("[Dert]: "..tostring(k).." isn't available.")
            end
...
if not shared.notified and shared.notificationlibinject == false then 
    ...
    notificationlib_v2({
        Title="Dert",
        Description="Dert has injected!",
        RBGShift=true,
    })
end 
getgenv().IS_DERT_LOADED = true
